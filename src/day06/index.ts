...
const part1 = (rawInput: string) => {
    ...
    function processInput() {
        ...
        for (const line of lines) {
            stringMap.push(line.trim().split(""));
        }
    ...
    function walkOnce() {
        if (direction == NORTH) {
            ...
            if (map[cursorRow][cursorCol] == BLOCKED) {
                cursorRow += 1;
                direction = EAST;
                return true;
            }
            map[cursorRow][cursorCol] = USED;
            return true;
        }
...
const part2 = (rawInput: string) => {
    ...
    function walkCandidateMaps() {
        ...
        while (true) {
            ...
            if (blockingCell == undefined) {
                return;
            }

            const blockingCellId = blockingCell.row + "~" + blockingCell.col;

            if (testedCells[blockingCellId] != undefined) {
                continue;
            }
    ...
    function walkCandidateMapSegment() {
        // checks only the spinning cells (the ones right before the blocked cells)

        if (direction == NORTH) {
            return walkCandidateMapNorth();
        }
        if (direction == SOUTH) {
            return walkCandidateMapSouth();
        }
        if (direction == EAST) {
            return walkCandidateMapEast();
        }
        if (direction == WEST) {
            return walkCandidateMapWest();
        }
    }
